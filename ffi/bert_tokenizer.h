...
void convert_pairs(void *handle, const char *text_a, const char *text_b, int max_seq_len);
...
class FullTokenizer{
 ...
 public:
    FullTokenizer(const std::string& vocab_file,int do_lower_case):handle(create_full_tokenizer(vocab_file.c_str(),do_lower_case)){
      if(handle == nullptr){
    ...
    void convert_pairs(const std::string& text_a,
